...
These packages can be installed or, after installing Anaconda, the following line can be run to set up a venv automatically:   
...
## Running 
Activate the python environment (`conda activate rocket-nav` if you're using andaconda)   

Run `rocket.py` to generate the trajectory, which will appear as `rocket.csv`.   
